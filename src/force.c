...
#pragma omp parallel for
  ...
  for (int i = 0; i < sph->N; i++) {
    ...
    sph->particles[i].factor = 1.0 / (1.0 + inv_dim * sph->particles[i].h / sph->particles[i].rho * sph->particles[i].drho_dh);
  }
  return;
}
...
__attribute__((always_inline)) static inline void compute_pairwise_physics_3d(Particle *p_i, Particle *p_j, SPHSystem *sph) {
    double dx = p_i->x - p_j->x;
    double dy = p_i->y - p_j->y;
    ...
    // --- 2. Thermal Energy Evolution (Pressure Part) ---
    // update the specific thermal energy evolution, Eq(25)
    double inner_product_v_dW_i = dvx * (dWdr_i * dx / r) + dvy * (dWdr_i * dy / r) + dvz * (dWdr_i * dz / r);
    double inner_product_v_dW_j = dvx * (dWdr_j * dx / r) + dvy * (dWdr_j * dy / r) + dvz * (dWdr_j * dz / r);
